...
public class ControlCaseEchange implements ControlCaseSpecial{
    ...
    public String appliquerEffet(Pirate pirate1, Pirate pirate2){
        ...
        return "Les positions de " + pirate1.getNom() + " et " 
                + pirate2.getNom() + " sont echangees. " + pirate1.getNom() +
                " est sur la case " + Integer.toString(pirate1.getPosition()) +
                " et " + pirate2.getNom() + " est sur la case " + 
                Integer.toString(pirate2.getPosition()) + "\n";
    }
}
